...
app.get('/api/xero-labour', async (req, res) => {
  ...
  const cookies = Object.fromEntries((req.headers.cookie || '').split('; ').filter(Boolean).map(c => { const [k, ...v] = c.split('='); return [k, v.join('=')]; }));
  ...
  if (!accessToken) {
    try {
      ...
      res.setHeader('Set-Cookie', [`xero_access_token=${accessToken}; HttpOnly; Path=/; Max-Age=${r.data.expires_in}; SameSite=Lax`, `xero_refresh_token=${r.data.refresh_token}; HttpOnly; Path=/; Max-Age=${60*60*24*30}; SameSite=Lax`]);
    } catch { return res.status(401).json({ error: 'Xero session expired — please reconnect' }); }
  }

  try {
    const LABOUR_KEYWORDS = ['wage', 'labour', 'labor', 'payroll', 'salary', 'salaries', 'superannuation'];
    const plRes = await axios.get(`https://api.xero.com/api.xro/2.0/Reports/ProfitAndLoss?fromDate=${fromDate}&toDate=${toDate}&standardLayout=true`,
      { headers: { Authorization: `Bearer ${accessToken}`, 'xero-tenant-id': tenantId, Accept: 'application/json' } }
    );
    const rows = plRes.data?.Reports?.[0]?.Rows || [];
    let labourExGST = 0;
    const walk = rows => { for (const row of rows) { if (row.Rows) walk(row.Rows); if (row.Cells) { const name = (row.Cells[0]?.Value || '').toLowerCase(); const amt = parseFloat(row.Cells[1]?.Value) || 0; if (LABOUR_KEYWORDS.some(k => name.includes(k))) labourExGST += Math.abs(amt); } } };
    walk(rows);
...
app.post('/api/parse-invoice', async (req, res) => {
  ...
  try {
    const response = await axios.post('https://api.anthropic.com/v1/messages',
      { model: 'claude-sonnet-4-20250514', max_tokens: 1000, messages: [{ role: 'user', content: [{ type: 'document', source: { type: 'base64', media_type: 'application/pdf', data: base64 } }, { type: 'text', text: 'Extract from this invoice and respond ONLY with valid JSON, no markdown:\n{"supplier":"<n>","invoice_number":"<inv#>","invoice_date":"<date>","total_inc_gst":<number>,"total_ex_gst":<number>,"gst_amount":<number>}\nUse null for missing fields.' }] }] },
