...
const Btn = ({ onClick, children, variant = 'dark', disabled, small }) => {
  const base = { border: 'none', borderRadius: 7, cursor: disabled ? 'not-allowed' : 'pointer', fontWeight: 600, opacity: disabled ? 0.6 : 1, fontSize: small ? 12 : 14, transition: 'opacity 0.15s' };
  const styles = {
    ...
    red: { ...base, background: '#dc2626', color: '#fff', padding: small ? '6px 10px' : '11px 18px' },
  };
...
const Field = ({ label, value, onChange, prefix = '$', readOnly }) => (
  <div style={{ marginBottom: 9 }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', border: `1px solid ${readOnly ? '#b7e2c8' : '#ddd'}`, borderRadius: 6, overflow: 'hidden', background: readOnly ? '#f0faf4' : '#fff' }}>
      ...
      {readOnly && <span style={{ fontSize: 10, color: '#16a34a', padding: '0 8px', whiteSpace: 'nowrap' }}>● auto</span>}
    </div>
  </div>
);
...
export default function App() {
  ...
  const [xeroStatus, setXeroStatus] = useState('disconnected'); // disconnected | connected | loading | error
  const [xeroLoading, setXeroLoading] = useState(false);
  const [aiReport, setAiReport] = useState('');
  const [aiLoading, setAiLoading] = useState(false);
  const [aiError, setAiError] = useState('');
  const pdfRef = useRef(); const lsRef = useRef();

  // Check if Xero is connected on load (via URL param)
  useEffect(() => {
  ...
  const labourEx = n('labourExGST'); // already ex-GST from Xero
  ...
  return (
    <div style={{ fontFamily: 'system-ui', maxWidth: 520, margin: '0 auto', padding: 20 }}>
      ...
      <p style={{ margin: '0 0 14px', color: '#888', fontSize: 12 }}>Revenue & COGS fields accept inc. GST — ex-GST calculated automatically.</p>
      ...
      {tab === 'import' && (
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 9, padding: 12, marginBottom: 10 }}>
            ...
            <div style={{ fontSize: 12, color: '#666', marginBottom: 8 }}>Fetches wages, super & payroll directly from your Xero P&L</div>
            {xeroStatus === 'connected' && (
          ...
          <div style={{ background: '#f8f9ff', border: '1px solid #dde', borderRadius: 9, padding: 12, marginBottom: 10 }}>
            ...
            <div style={{ fontSize: 12, color: '#666', marginBottom: 8 }}>Imports Turnover (Coffee & Food) + Transactions</div>
      ...
      <Field label="Turnover — Food & Bev (inc. GST)" value={merged.turnoverFoodInc} onChange={set('turnoverFoodInc')} readOnly={isAuto('turnoverFoodInc')} />
      ...
      <Field label="COGS — Food & Bev (inc. GST)" value={merged.cogsFoodInc} onChange={set('cogsFoodInc')} readOnly={isAuto('cogsFoodInc')} />
      ...
      <Field label="Labour — Wages, Super & Payroll (ex. GST)" value={merged.labourExGST} onChange={set('labourExGST')} readOnly={isAuto('labourExGST')} prefix="$" />
